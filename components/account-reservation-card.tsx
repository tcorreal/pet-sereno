...
export function AccountReservationCard({ reservation }: { reservation: Record<string, unknown> }) {
  ...
  return <article className="reservation-item"><div className="reservation-item__top"><div><code>{String(reservation.service_number || reservation.reservation_number)}</code><h3>{String(reservation.pet_name)}</h3><p>{String(reservation.service_type)}</p></div><StatusBadge status={status}/></div>
    <dl><div><dt>Ingreso</dt><dd suppressHydrationWarning>{new Date(String(reservation.start_datetime)).toLocaleString("es-CO", { dateStyle: "medium", timeStyle: "short", timeZone: "America/Bogota" })}</dd></div><div><dt>Salida</dt><dd suppressHydrationWarning>{new Date(String(reservation.end_datetime)).toLocaleString("es-CO", { dateStyle: "medium", timeStyle: "short", timeZone: "America/Bogota" })}</dd></div><div><dt>Reservó</dt><dd>{String(reservation.created_by || "Registro anterior")}</dd></div></dl>
    ...
    {code && <div className="temporary-code" role="status"><span>{code.operation === "DROPOFF" ? "ENTREGA" : "RECOGIDA"}</span><strong>{code.value}</strong><small>Válido hasta {new Date(code.expiresAt).toLocaleTimeString("es-CO", { hour: "numeric", minute: "2-digit", timeZone: "America/Bogota" })}. Un solo uso.</small></div>}{error && <div className="alert alert--error">{error}</div>}
  </article>;
}
